          ...
          fluidRow(
            ...
            column(
              ...
              bsPopover(
                ...
                content = paste0("The Davis Instruments sensor measures on a scale of 0 (fully wet) to 200 (fully dry) centibars.  The sensor measures the vacuum created in the soil by the lack of moisture. A reading of -1 means that the sensor does not return any reading."),
                placement = "right",
                trigger = "click",
...
server <- function(input, output, session) {
  ...
  #--- Filter data frame based on selected date range
  ...
  all_weather <- reactive({
    fields <- '{"date_and_time":1, "date":1, "soil_moisture_1": { "$ifNull" : ["$davis_current_observation.soil_moisture_1", "N/A"]}}'
  ...
  output$week_soil <- renderPlotly({
    ...
    by_day <- df_temp %>%
      group_by(date) %>%
      summarise(avg_moisture = mean(soil_moisture_1), na.rm = TRUE) %>%
      filter(is.na(avg_moisture) == FALSE)

    day_avg <- round((obs$soil_moisture_1_day_high + obs$soil_moisture_1_day_low) / 2, 0)
    day_avg <- ifelse(is.na(day_avg), 0, day_avg)

    all_time_avg <- round(mean(df_temp$soil_moisture_1, na.rm = TRUE), 0)
    all_time_avg <- ifelse(is.na(all_time_avg), 0, all_time_avg)
